...
// Select all products between IW-JW-186 and IW-JW-226
$stmt = $db->prepare("SELECT id, sku, name FROM products WHERE sku >= 'IW-JW-186' AND sku <= 'IW-JW-226' ORDER BY id ASC");
$stmt->execute();
...
if (empty($ids)) {
    echo "No products found in range IW-JW-186 to IW-JW-226.\n";
    exit;
}
...
foreach ($childTables as $t) {
    try {
        ...
        // Table might not exist or might fail silently
    }
}
