...
<div class="everful-card" id="<?= $cardId ?>" data-product-id="<?= $product['id'] ?? 0 ?>">
  ...
  <div class="ef-img-stage">
    ...
    <!-- Bottom-Right: Toggle Inquiry Button -->
    <button type="button" 
            class="ef-icon-btn ef-cart-btn ef-inquiry-btn <?= $isInInquiry ? 'active' : '' ?>" 
            onclick="toggleCardInquiry(<?= $product['id'] ?? 0 ?>, <?= $moq ?>, this)" 
            title="<?= $isInInquiry ? 'In Inquiry (Click to remove)' : 'Add to Inquiry' ?>">
      <svg class="ef-inquiry-icon" width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <?php if ($isInInquiry): ?>
