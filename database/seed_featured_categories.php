...
try {
    ...
    echo "========================================================\n";
    echo "1. Deleting existing Featured Categories & Subcategories...\n";
    echo "========================================================\n";
    ...
    echo "========================================================\n";
    echo "2. Seeding Fresh Featured Categories & Subcategories with Images...\n";
    echo "========================================================\n";
    ...
    // Ensure image exists locally, download if missing
    $ensureImage = function($filename, $remoteUrl) use ($uploadDir) {
        ...
        if (!file_exists($localPath)) {
            ...
            if ($data && strlen($data) > 1000) {
                ...
                // If download fails, copy fallback from existing image
    ...
    foreach ($tabs as $t) {
        $catDbImagePath = $ensureImage($t['filename'], $t['remote_url']);

        $ins = $db->prepare("INSERT INTO featured_categories (name, slug, image, sort_order, is_active) VALUES (?, ?, ?, ?, 1)");
        $ins->execute([$t['name'], $t['slug'], $catDbImagePath, $t['sort_order']]);
        $catId = $db->lastInsertId();
        echo "Inserted Category [ID {$catId}]: {$t['name']} (image: {$catDbImagePath})\n";

        foreach ($t['subcategories'] as $sub) {
            $subDbImagePath = $ensureImage($sub['filename'], $sub['remote_url']);

            $insSub = $db->prepare("INSERT INTO featured_subcategories (featured_category_id, name, slug, image, link_url, sort_order, is_active) VALUES (?, ?, ?, ?, ?, ?, 1)");
            $insSub->execute([$catId, $sub['name'], $sub['slug'], $subDbImagePath, $sub['link_url'], $sub['sort_order']]);
            echo "  + Subcategory [ID " . $db->lastInsertId() . "]: {$sub['name']} (image: {$subDbImagePath})\n";
        }
    }

    echo "\n========================================================\n";
    echo "DELETION & FRESH SEEDING COMPLETED SUCCESSFULLY!\n";
    echo "========================================================\n";
