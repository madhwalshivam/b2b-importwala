    ...
    <div id="admin-confirm-modal" class="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-xs transition-opacity duration-200 hidden">
        <div id="admin-confirm-modal-box" class="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl shadow-2xl max-w-md w-full mx-4 p-6 space-y-5 transform transition-all duration-200 scale-95">
            <div class="flex items-start space-x-4">
                <div class="w-12 h-12 rounded-2xl bg-red-50 dark:bg-red-950/50 border border-red-200 dark:border-red-900/50 text-red-600 dark:text-red-400 flex items-center justify-center shrink-0">
                    <i data-lucide="alert-triangle" class="w-6 h-6 text-red-600 dark:text-red-400"></i>
                </div>
                <div class="space-y-1 min-w-0 flex-1">
                    <h3 id="admin-confirm-title" class="text-base font-bold text-gray-900 dark:text-white">Confirm Action</h3>
                    <p id="admin-confirm-message" class="text-xs text-gray-600 dark:text-slate-400 leading-relaxed">Are you sure you want to proceed?</p>
                </div>
            </div>

            <div id="admin-confirm-preview" class="hidden justify-center p-2 bg-gray-50 dark:bg-slate-800/60 rounded-xl border border-gray-200 dark:border-slate-700">
    ...
    <div id="admin-toast-container" class="fixed bottom-5 right-5 z-[99999] flex flex-col space-y-2 pointer-events-none"></div>
    ...
    <script>
        ...
        window.showToast = function(message, type = 'success') {
            ...
            toast.className = `pointer-events-auto flex items-center space-x-3 px-4 py-3 rounded-xl border shadow-xl transition-all duration-300 transform translate-y-4 opacity-0 text-xs font-semibold ${
                isSuccess 
