...
<div class="space-y-6 font-sans">
    ...
    <div
        ...
        <div>
            ...
            <p class="text-xs text-slate-500 mt-0.5 font-medium max-w-2xl">
                Control top header menu links (Home, Categories, New Arrivals, Support, etc.), dropdown submenus, URLs,
                position order, and hide/show status dynamically.
            </p>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-xs p-6">
        <div class="flex items-center justify-between mb-5 pb-3 border-b border-slate-100">
            ...
            <span class="text-xs text-slate-400 font-medium">Reorder using Up/Down arrows or 1-click status
                toggle</span>
        </div>
                    ...
                    <div class="bg-white hover:bg-slate-50/60 border border-slate-200 rounded-xl p-4 transition shadow-2xs">
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                            ...
                            <div class="flex items-center space-x-3 flex-1 min-w-0">
                                <span
                                    class="flex-shrink-0 w-8 h-8 rounded-lg bg-slate-100 border border-slate-200 text-slate-700 text-xs font-semibold flex items-center justify-center">
                                    #<?= $parent['sort_order'] ?>
                                </span>

                                <div class="min-w-0 flex-1">
                                    <div class="flex items-center space-x-2 flex-wrap gap-y-1">
                                        <span class="text-sm font-semibold text-slate-900 tracking-wide truncate">
                                            <?= htmlspecialchars($parent['label']) ?>
                                        </span>

                                        <!-- Type Badge -->
                                        <span
                                            class="px-2 py-0.5 text-[10px] font-semibold uppercase rounded-md tracking-wider border <?= $parent['type'] === 'dropdown' ? 'bg-purple-50 text-purple-700 border-purple-200' : ($parent['type'] === 'category' ? 'bg-blue-50 text-blue-700 border-blue-200' : 'bg-slate-100 text-slate-700 border-slate-200') ?>">
                                        ...
                                        <?php if (!empty($parent['open_in_new_tab'])): ?>
                                            <span
                                                class="px-1.5 py-0.5 text-[9px] font-medium bg-amber-50 text-amber-700 border border-amber-200 rounded"
                                        ...
                                        <!-- Active Status Badge -->
                                        <span
                                            class="px-2 py-0.5 text-[10px] font-semibold rounded-full border <?= !empty($parent['is_active']) ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-rose-50 text-rose-700 border-rose-200' ?>">
                            ...
                            <div class="flex items-center space-x-1.5 shrink-0 self-end sm:self-center">
                                ...
                                <button type="button"
                                    onclick="openAddSubmenuModal(<?= $parent['id'] ?>, '<?= htmlspecialchars(addslashes($parent['label'])) ?>')"
                                ...
                                <!-- Edit Link Button -->
                                <button type="button"
                                    onclick='openEditModal(<?= json_encode($parent, JSON_HEX_APOS | JSON_HEX_QUOT) ?>)'
                                    class="p-1.5 bg-blue-50 hover:bg-blue-100 text-blue-600 rounded-lg border border-blue-200 transition cursor-pointer"
                                    title="Edit Link">
                                    <i data-lucide="edit-3" class="w-4 h-4"></i>
                                </button>

                                <!-- Delete Link Button -->
                                <form action="<?= url("admin/navigation/delete/{$parent['id']}") ?>" method="POST"
                                    class="inline"
                                    onsubmit="return confirm('Are you sure you want to delete \'<?= htmlspecialchars(addslashes($parent['label'])) ?>\'? Any dropdown sub-items will also be deleted.')">
                                    <?= csrf_field() ?>
                                    <button type="submit"
                        ...
                        <?php if (!empty($parent['children'])): ?>
                            <div class="mt-3 ml-4 sm:ml-8 pl-4 border-l-2 border-slate-200 space-y-2 pt-2">
                                <div
                                    class="text-[11px] font-semibold uppercase text-slate-400 tracking-wider mb-2 flex items-center gap-1.5">
                                ...
                                <?php foreach ($parent['children'] as $child): ?>
                                    <div
                                        class="bg-slate-50 hover:bg-slate-100/70 border border-slate-200/80 rounded-lg p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 transition">

                                        <div class="flex items-center space-x-3 min-w-0">
                                            <span class="text-slate-400 text-xs font-mono font-semibold">
                                                ↳ #<?= $child['sort_order'] ?>
                                            </span>
                                            <div class="min-w-0">
                                                <div class="flex items-center space-x-2 flex-wrap">
                                                    ...
                                                    <span
                                                        class="px-1.5 py-0.2 text-[9px] font-mono rounded border <?= $child['type'] === 'category' ? 'bg-blue-50 text-blue-700 border-blue-200' : 'bg-slate-100 text-slate-600 border-slate-200' ?>">
                                                        <?= htmlspecialchars($child['type']) ?>
                                                    </span>
                                                    <span
                                                        class="px-1.5 py-0.2 text-[9px] font-semibold rounded <?= !empty($child['is_active']) ? 'text-emerald-700' : 'text-rose-600' ?>">
                                        ...
                                        <div class="flex items-center space-x-1 shrink-0 self-end sm:self-center">
                                            <a href="<?= url("admin/navigation/move/{$child['id']}/up") ?>"
                                                class="p-1 text-slate-500 hover:text-slate-800" title="Move Up"><i
                                                    data-lucide="chevron-up" class="w-3.5 h-3.5"></i></a>
                                            <a href="<?= url("admin/navigation/move/{$child['id']}/down") ?>"
                                                class="p-1 text-slate-500 hover:text-slate-800" title="Move Down"><i
                                                    data-lucide="chevron-down" class="w-3.5 h-3.5"></i></a>
                                            <a href="<?= url("admin/navigation/toggle-status/{$child['id']}") ?>"
                                                class="px-2 py-0.5 text-[11px] font-semibold rounded border <?= !empty($child['is_active']) ? 'bg-amber-50 text-amber-700 border-amber-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200' ?>">
                                                <?= !empty($child['is_active']) ? 'Hide' : 'Show' ?>
                                            </a>
                                            <button type="button"
                                                onclick='openEditModal(<?= json_encode($child, JSON_HEX_APOS | JSON_HEX_QUOT) ?>)'
                                                class="p-1 text-blue-600 hover:text-blue-800" title="Edit">
                                                <i data-lucide="edit-2" class="w-3.5 h-3.5"></i>
                                            </button>
                                            <form action="<?= url("admin/navigation/delete/{$child['id']}") ?>" method="POST"
                                                class="inline" onsubmit="return confirm('Delete submenu item?')">
                                                <?= csrf_field() ?>
...
<div id="addNavModal"
    ...
    <div
        class="bg-white border border-slate-200 rounded-2xl w-full max-w-xl shadow-2xl overflow-hidden animate-scale-in">
        ...
        <form action="<?= url('admin/navigation/store') ?>" method="POST" class="p-6 space-y-4">
            ...
            <div>
                ...
                <select id="addPresetSelect" onchange="applyPreset('add', this.value)"
                    ...
                    <option value="catalog" data-label="All Categories" data-url="/catalog" data-type="dropdown">
                        Categories Catalog (/catalog)</option>
                    <option value="new_arrivals" data-label="New Arrivals" data-url="/catalog?sort=newest"
                        data-type="internal">New Arrivals (/catalog?sort=newest)</option>
                    <option value="best_sellers" data-label="Best Sellers" data-url="/catalog?sort=popular"
                        data-type="internal">Best Sellers (/catalog?sort=popular)</option>
                    <option value="free_shipping" data-label="Free Air Shipping" data-url="/catalog?free_shipping=1"
                        data-type="internal">Free Air Shipping (/catalog?free_shipping=1)</option>
                    <option value="price_drops" data-label="Price Drops" data-url="/catalog?price_drops=1"
                        data-type="internal">Price Drops (/catalog?price_drops=1)</option>
                    <option value="blog" data-label="Blog" data-url="/blog" data-type="internal">Blog (/blog)</option>
                    <option value="support" data-label="Support" data-url="/support" data-type="dropdown">Support
                        (/support)</option>
                    <?php if (!empty($categories)): ?>
                        <optgroup label="Product Categories">
                            <?php foreach ($categories as $cat): ?>
                                <option value="cat_<?= $cat['id'] ?>" data-label="<?= htmlspecialchars($cat['name']) ?>"
                                    data-url="/catalog?category=<?= urlencode($cat['slug']) ?>" data-type="category">
            ...
            <div>
                ...
                <input type="text" name="url" id="addUrl" required
                    placeholder="e.g. /catalog?clearance=1 or https://example.com"
            ...
            <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-2">
                ...
                <button type="submit"
                    class="px-5 py-2 bg-red-600 hover:bg-red-700 text-white text-xs font-semibold rounded-xl shadow-xs transition">
    ...
    <div
        ...
        <form id="editForm" action="" method="POST" class="p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div class="flex items-center pt-5">
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="open_in_new_tab" id="editOpenInNewTab" value="1"
                            class="sr-only peer">
                        <div
                            class="w-9 h-5 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-red-600 relative">
                        </div>
            ...
            <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-2">
                ...
                <button type="submit"
                    class="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-xl shadow-xs transition">
