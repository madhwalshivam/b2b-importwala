...
<div class="space-y-5 font-sans pb-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-5 rounded-2xl border border-gray-200/80 shadow-xs">
        <div>
            <div class="flex items-center space-x-2">
                <span class="px-2.5 py-0.5 text-[10px] font-extrabold uppercase bg-orange-50 text-[#f05a29] rounded-md tracking-wider border border-orange-200">
                    Catalog &amp; Products
                </span>
        ...
        <?php if (\App\Core\Auth::hasPermission('products.add')): ?>
            <a href="<?= url('admin/products/create') ?>"
                class="h-9 px-4 bg-[#f05a29] hover:bg-[#d8481b] text-white font-bold text-xs rounded-xl transition shadow-sm shadow-[#f05a29]/30 flex items-center space-x-1.5 cursor-pointer shrink-0">
    ...
    <!-- Filter & Search Bar -->
    <div class="bg-white p-3.5 rounded-2xl border border-gray-200/80 shadow-xs flex flex-wrap items-center justify-between gap-3">
        <form action="<?= url('admin/products') ?>" method="GET" class="flex flex-wrap items-center gap-2.5 flex-1">
            ...
            <button type="submit"
                class="h-9 px-4 bg-slate-900 text-white text-xs font-bold rounded-xl hover:bg-black transition cursor-pointer">Filter</button>
        </form>
    ...
    <div class="bg-white rounded-2xl border border-gray-200/80 shadow-xs overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-xs text-left border-collapse min-w-[840px]">
                <thead>
                    <tr class="bg-slate-900 text-white font-bold uppercase tracking-wider text-[10px] select-none">
                ...
                <tbody class="divide-y divide-gray-100/80">
                    <?php if (empty($products)): ?>
                        <tr>
                            <td colspan="11" class="py-8 text-center text-gray-400 font-medium text-xs">No products found.</td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($products as $p): ?>
                            ...
                            ?>
                            <tr class="hover:bg-slate-50/70 transition <?= $p['status'] === 'merged' ? 'bg-purple-50/30' : '' ?>">
                                ...
                                <td class="py-2.5 px-3 flex items-center space-x-2.5 min-w-[240px]">
                                    <div class="w-10 h-10 rounded-lg bg-slate-100 border border-slate-200 overflow-hidden shrink-0 flex items-center justify-center p-0.5">
                                    ...
                                    <div>
                                        <h4 class="font-bold text-slate-900 line-clamp-1 text-[11px] leading-snug">
                                ...
                                <td class="py-2.5 px-3 font-mono font-semibold text-slate-600 text-[11px]"><?= htmlspecialchars($p['sku']) ?></td>
                                <td class="py-2.5 px-3 font-extrabold text-slate-900 text-[11px]"><?= format_price($p['sale_price'] ?: $p['price']) ?></td>

                                <!-- Professional iOS Toggle: New Product -->
                                <td class="py-2.5 px-3 text-center">
                                    <button type="button" 
                                            onclick="quickToggleSwitch(<?= $p['id'] ?>, 'is_new', this)" 
                                        ...
                                        <span class="w-3.5 h-3.5 rounded-full bg-white transition-transform duration-200 ease-in-out shadow-xs transform <?= $isNew ? 'translate-x-3.5' : 'translate-x-0' ?>"></span>
                                    </button>
                                </td>
                                ...
                                <td class="py-2.5 px-3 text-center">
                                    <button type="button" 
                                            onclick="quickToggleSwitch(<?= $p['id'] ?>, 'is_free_shipping', this)" 
                                        ...
                                        <span class="w-3.5 h-3.5 rounded-full bg-white transition-transform duration-200 ease-in-out shadow-xs transform <?= $isFreeShipping ? 'translate-x-3.5' : 'translate-x-0' ?>"></span>
                                    </button>
                                </td>

                                <td class="py-2.5 px-3 text-center font-bold text-[11px] text-slate-800">
                                    <?= number_format($soldCount) ?>
                                </td>

                                <td class="py-2.5 px-3 text-center font-bold text-[11px] text-orange-600">
                                    <?= $moqCount ?> pcs
                                </td>

                                <td class="py-2.5 px-3">
                                    <?php if ($p['status'] === 'active'): ?>
                                        <span class="px-2 py-0.5 text-[9px] font-bold rounded-md uppercase bg-emerald-100 text-emerald-700 border border-emerald-200">Active</span>
                                    <?php elseif ($p['status'] === 'merged'): ?>
                                        <span class="px-2 py-0.5 text-[9px] font-bold rounded-md uppercase bg-purple-100 text-purple-700 border border-purple-200">Merged</span>
                                    <?php else: ?>
                                        <span class="px-2 py-0.5 text-[9px] font-bold rounded-md uppercase bg-gray-100 text-gray-600 border border-gray-200">Inactive</span>
                                    <?php endif; ?>
                                </td>
                                    ...
                                    <div class="flex items-center justify-end space-x-1.5">
                                        <?php if (\App\Core\Auth::hasPermission('products.edit')): ?>
                                            <a href="<?= url('admin/products/edit/' . $p['id']) ?>"
                                                class="px-2.5 py-1 bg-slate-900 text-white rounded-md font-bold text-[10px] hover:bg-black transition shadow-2xs">Edit</a>
                                        <?php endif; ?>
                                        <?php if (\App\Core\Auth::hasPermission('products.delete')): ?>
                                            <form action="<?= url('admin/products/delete/' . $p['id']) ?>" method="POST"
                                                ...
                                                <?= csrf_field() ?>
                                                <button type="submit"
                                                    class="px-2.5 py-1 bg-red-50 text-red-600 rounded-md font-bold text-[10px] hover:bg-red-600 hover:text-white transition shadow-2xs inline-flex items-center space-x-1"
