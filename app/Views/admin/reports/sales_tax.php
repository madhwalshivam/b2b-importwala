...
<div class="space-y-6 font-sans pb-12">
        ...
        <div class="flex items-center space-x-3.5">
            ...
            <div>
                <div class="flex items-center space-x-2">
                    <span
                        class="px-2.5 py-0.5 text-[10px] font-semibold uppercase bg-red-50 text-red-600 rounded-md tracking-wider border border-red-100">
                        Financial Intelligence
                    </span>
                    <span class="text-slate-400 text-xs">•</span>
                    <span class="text-xs text-slate-500 font-medium"><?= htmlspecialchars($filter_label) ?></span>
                </div>
                <h1 class="text-xl font-semibold text-slate-900 tracking-tight mt-0.5">Monthly Sales & Tax Report</h1>
                <p class="text-xs text-slate-500 mt-0.5">GST tax collection breakdown, gross & net sales summary, and
                    itemized transaction log.</p>
            </div>
        </div>
    ...
    <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs space-y-4">
        <div class="flex items-center justify-between border-b border-slate-100 pb-3">
            <div class="flex items-center space-x-2">
                ...
                <h3 class="text-xs font-semibold text-slate-800 uppercase tracking-wider">Report Filter Criteria</h3>
            </div>
            <?php if (!empty($fromDate) || !empty($toDate) || $selectedMonth !== (int) date('n') || $selectedYear !== (int) date('Y')): ?>
                <a href="<?= url('admin/reports/sales-tax') ?>"
                    class="text-[11px] font-semibold text-red-600 hover:underline flex items-center space-x-1">
        ...
        <form action="<?= url('admin/reports/sales-tax') ?>" method="GET"
            class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 text-xs">

            <!-- Month Dropdown -->
            <div>
                <label class="block font-semibold text-slate-700 mb-1">Select Month</label>
                <select name="month"
                    class="w-full h-10 px-3 bg-slate-50 border border-slate-200 rounded-xl font-medium text-slate-900 focus:outline-none focus:border-red-600 focus:bg-white transition cursor-pointer">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
            <div>
                <span class="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Total Orders</span>
                <h3 class="text-2xl font-extrabold text-slate-900 mt-1"><?= number_format($stats['total_orders']) ?>
                </h3>
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
            <div>
                <span class="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Gross Sales</span>
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
            <div>
                <span class="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">GST Tax Collected</span>
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
            <div>
                <span class="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Net Revenue</span>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
        <div class="p-5 border-b border-slate-100 flex items-center justify-between">
            <div>
                <h3 class="text-sm font-semibold text-slate-900">Order-wise Financial Breakdown</h3>
                <p class="text-xs text-slate-500 mt-0.5">Itemized transaction list for
                    <strong><?= htmlspecialchars($filter_label) ?></strong></p>
            </div>
            <span
                class="text-xs font-semibold bg-slate-100 text-slate-700 px-3 py-1 rounded-lg border border-slate-200">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-xs">
                        ...
                        <tr>
                            <td colspan="9" class="p-12 text-center text-slate-400">
                                <div class="flex flex-col items-center justify-center space-y-2">
                                    ...
                                    <p class="text-xs text-slate-400">No orders were created during the selected date range
                                        (<?= htmlspecialchars($filter_label) ?>).</p>
                                </div>
                            </td>
                        </tr>
                    <?php else: ?>
                            ...
                            <tr class="hover:bg-slate-50/70 transition <?= $isCancelled ? 'bg-rose-50/40 opacity-75' : '' ?>">
                                ...
                                <td class="py-3 px-3">
                                    <div class="font-semibold text-slate-900">
                                        <?= htmlspecialchars($ord['customer_name'] ?? 'N/A') ?></div>
                                    <div class="text-[11px] text-slate-400 font-mono">
                                        <?= htmlspecialchars($ord['customer_phone'] ?? '') ?></div>
                                </td>
                                <td class="py-3 px-3 text-center">
                                    <span
                                        class="font-mono text-xs font-semibold bg-slate-100 border border-slate-200 px-2 py-0.5 rounded text-slate-800">
                                        <?= htmlspecialchars($ord['hsn_codes'] ?? '8714.99.90') ?>
                                    </span>
                                </td>
                                <td class="py-3 px-3 text-center">
                                    <span
                                        class="font-mono text-xs font-semibold bg-red-50 text-red-700 border border-red-200 px-2 py-0.5 rounded">
                                ...
                                <td class="py-3 px-3 text-center">
                                    ...
                                    ?>
                                    <span
                                        class="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] uppercase font-semibold border <?= $ordClass ?>">
                                        <?= htmlspecialchars($ord['order_status'] ?? 'pending') ?>
                                    </span>
                                </td>
                                <td class="py-3 px-3 text-right font-mono text-slate-900 font-semibold">
                                ...
                                <td class="py-3 px-3 text-right font-mono text-slate-700 font-semibold">
                                    <?= ((float) ($ord['shipping_charge'] ?? 0) <= 0) ? '<span class="text-emerald-700 font-semibold bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-200 text-[10px]">FREE</span>' : format_price($ord['shipping_charge']) ?>
                                </td>
                                <td class="py-3 px-4 text-right pr-6 font-mono font-semibold text-slate-900">
                ...
                <?php if (!empty($orders)): ?>
                    <tfoot class="bg-slate-100/90 border-t-2 border-slate-300 font-semibold text-xs text-slate-900">
                        <tr>
