...
<div class="space-y-5 font-sans pb-8">
    ...
    <div
        ...
        <div>
            ...
            <p class="text-xs text-gray-500 font-medium mt-0.5">Manage buyer custom sourcing requirements, follow up via
                WhatsApp, and track status.</p>
        </div>

        <div class="flex items-center gap-2 shrink-0">
            <?php if (!empty($newCount) && $newCount > 0): ?>
                <span
                    class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#f05a29] text-white text-xs font-semibold rounded-xl shadow-xs animate-pulse">
            ...
            <a href="<?= url('admin/rfq/export-csv?' . http_build_query(['search' => $search, 'status' => $status, 'date_from' => $dateFrom, 'date_to' => $dateTo])) ?>"
                class="h-9 px-4 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-xs rounded-xl shadow-xs transition flex items-center gap-1.5">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white p-4 rounded-2xl border border-gray-200/80 shadow-xs flex items-center justify-between">
            <div>
                <span class="text-[11px] font-semibold text-slate-500 uppercase tracking-wider block">New
                    Requests</span>
                <span
        ...
        <div class="bg-white p-4 rounded-2xl border border-gray-200/80 shadow-xs flex items-center justify-between">
            <div>
                <span class="text-[11px] font-semibold text-slate-500 uppercase tracking-wider block">WhatsApp
                    Direct</span>
                <span class="text-xs font-semibold text-emerald-600 mt-0.5 block">1-Click Contact</span>
            </div>
            <div
    ...
    <div class="bg-white p-4 rounded-2xl border border-gray-200/80 shadow-xs">
        <form action="<?= url('admin/rfq') ?>" method="GET"
            ...
            <div class="lg:col-span-2">
                <label class="block text-[11px] font-semibold text-slate-600 mb-1 uppercase tracking-wider">From
                    Date</label>
            ...
            <div class="lg:col-span-2">
                <label class="block text-[11px] font-semibold text-slate-600 mb-1 uppercase tracking-wider">To
                    Date</label>
                        ...
                        <?php foreach ($rfqs as $rfq): ?>
                            ...
                            <tr class="hover:bg-slate-50/60 transition group">
                                ...
                                <td class="py-3.5 px-5 max-w-[240px]">
                                    <div class="font-semibold text-slate-900 text-xs truncate"
                                        title="<?= htmlspecialchars($rfq['product_name']) ?>">
                                ...
                                <td class="py-3.5 px-5 text-center whitespace-nowrap">
                                    ...
                                    <?php if (!empty($rfq['overall_budget'])): ?>
                                        <div class="text-[11px] font-normal text-slate-500">
                                            <?= htmlspecialchars($rfq['overall_budget']) ?></div>
                                    <?php endif; ?>
                                </td>

                                <td class="py-3.5 px-5 whitespace-nowrap">
                                    <div class="font-semibold text-slate-900 text-xs"><?= htmlspecialchars($rfq['full_name']) ?>
                                    </div>
                                    <?php if (!empty($rfq['business_type'])): ?>
                                        <div class="text-[11px] text-slate-500 font-normal">
                                            <?= htmlspecialchars($rfq['business_type']) ?></div>
                                    <?php endif; ?>
                                </td>
                                ...
                                <td class="py-3.5 px-5 whitespace-nowrap">
                                    ...
                                    <?php if (!empty($rfq['email'])): ?>
                                        <div class="text-[11px] text-slate-400 font-normal truncate max-w-[180px]">
                                            <?= htmlspecialchars($rfq['email']) ?></div>
                                    <?php endif; ?>
                                </td>

                                <td class="py-3.5 px-5 text-center whitespace-nowrap">
                                    <span
                                        class="inline-flex items-center px-2.5 py-0.5 text-[11px] font-semibold rounded-full border <?= $scClass ?>">
