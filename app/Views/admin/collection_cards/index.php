...
<div class="space-y-6">
    ...
    <div
        ...
        <div>
            ...
            <p class="text-xs text-slate-500 mt-0.5 font-medium max-w-2xl">
                "Trending Collections" jaisi cards manage karo. Har card mein title, image aur manually selected
                products hote hain.
            </p>
        </div>
    ...
    <?php if (empty($cards)): ?>
        <div
            class="bg-white rounded-2xl border border-slate-200 shadow-xs p-16 flex flex-col items-center justify-center text-center gap-4">
        ...
        <div class="grid grid-cols-1 xl:grid-cols-2 gap-6" id="cardsContainer">
            <?php foreach ($cards as $card):
                ...
                <div class="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden" data-card-id="<?= $cid ?>">
                    ...
                    <div class="flex items-start gap-4 p-5 border-b border-slate-100">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 flex-wrap">
                                <span
                                    class="font-semibold text-sm text-slate-900 truncate"><?= htmlspecialchars($card['title']) ?></span>
                                <?php if (!empty($card['badge_text'])): ?>
                                    <span
                                        class="px-2 py-0.5 text-[10px] font-semibold bg-amber-50 text-amber-700 rounded-full border border-amber-200">
                        ...
                        <div class="flex items-center gap-1 shrink-0">
                            ...
                            <form action="<?= url('admin/collection-cards/delete/' . $cid) ?>" method="POST"
                                onsubmit="return confirm('\" <?= htmlspecialchars(addslashes($card['title'])) ?>\" aur uske sab
                                products delete ho jayenge. Sure hai?')">
                                <?= csrf_field() ?>
                                <button type="submit"
                    ...
                    <div class="p-5 space-y-3">
                        <div class="flex items-center justify-between">
                            <span class="text-xs font-semibold text-slate-700 flex items-center gap-1.5">
                                ...
                                <span class="px-1.5 py-0.5 bg-slate-100 text-slate-600 rounded-md text-[10px] font-semibold"
                                    id="count-<?= $cid ?>">
                                    <?= count($assignedProducts) ?>
                                </span>
                            </span>
                            <button
                                onclick="openProductModal(<?= $cid ?>, '<?= htmlspecialchars(addslashes($card['title'])) ?>')"
                        ...
                        <div id="preview-<?= $cid ?>" class="flex flex-wrap gap-2">
                            <?php if (empty($assignedProducts)): ?>
                                <p class="text-xs text-slate-400 italic" id="empty-<?= $cid ?>">Koi product assign nahi. "Manage
                                    Products" click karo.</p>
                            <?php else: ?>
                                <?php foreach (array_slice($assignedProducts, 0, 8) as $ap): ?>
                                    <div
                                        class="flex items-center gap-1.5 px-2.5 py-1 bg-slate-50 border border-slate-200 rounded-lg text-[11px] text-slate-700 font-medium">
                                ...
                                <?php if (count($assignedProducts) > 8): ?>
                                    <span
                                        class="px-2.5 py-1 bg-red-50 text-red-600 border border-red-100 rounded-lg text-[11px] font-semibold">
                    ...
                    <input type="hidden" id="assigned-<?= $cid ?>" value="<?= htmlspecialchars(json_encode($assignedIds)) ?>">
                </div>
            <?php endforeach; ?>
        </div>
    <?php endif; ?>
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-xl overflow-hidden">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <div class="flex items-center gap-2">
                ...
                <h3 class="text-sm font-semibold text-slate-900">New Collection Card</h3>
            </div>
        ...
        <form action="<?= url('admin/collection-cards/store') ?>" method="POST" enctype="multipart/form-data"
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="col-span-2">
                    <label class="block text-xs font-semibold text-slate-700 mb-1">Card Title <span
            ...
            <div class="flex justify-end gap-3 pt-2 border-t border-slate-100">
                ...
                <button type="submit"
                    class="px-5 py-2 bg-red-600 text-white rounded-xl text-xs font-semibold hover:bg-red-700 transition cursor-pointer">
                    Create Card
                </button>
            </div>
        </form>
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-xl overflow-hidden">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            <div class="flex items-center gap-2">
                ...
                <h3 class="text-sm font-semibold text-slate-900">Edit Collection Card</h3>
            </div>
        ...
        <form id="editForm" action="" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
            ...
            <div class="flex justify-end gap-3 pt-2 border-t border-slate-100">
                ...
                <button type="submit"
                    class="px-5 py-2 bg-slate-900 text-white rounded-xl text-xs font-semibold hover:bg-slate-700 transition cursor-pointer">
                    Save Changes
                </button>
            </div>
        </form>
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col"
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100 shrink-0">
            <div class="flex items-center gap-2">
                ...
                <div>
                    <h3 class="text-sm font-semibold text-slate-900">Manage Products</h3>
                    <p class="text-[11px] text-slate-500" id="productModalSubtitle">Card ke liye products select karo
                    </p>
                </div>
            </div>
            ...
            <div class="w-1/2 border-r border-slate-100 flex flex-col">
                ...
                <div class="p-4 border-b border-slate-100 shrink-0">
                    <div class="relative">
                        ...
                        <input type="text" id="productSearch" placeholder="Product naam ya SKU se search karo..."
                ...
                <div class="flex-1 overflow-y-auto p-3 space-y-1" id="productSearchResults">
                    <p class="text-xs text-slate-400 text-center py-8">Search karo ya sab products dekhne ke liye kuch
                        type karo...</p>
                </div>
            </div>
                ...
                <div class="px-4 py-3 border-b border-slate-100 shrink-0 flex items-center justify-between">
                    <span class="text-xs font-semibold text-slate-700 flex items-center gap-1.5">
                        ...
                        <span id="selectedCount"
                            class="px-1.5 py-0.5 bg-red-100 text-red-600 rounded text-[10px] font-semibold">0</span>
                    </span>
                    <button onclick="clearAllSelected()"
                        class="text-[10px] text-slate-400 hover:text-red-600 transition cursor-pointer font-medium">
                        Clear All
                    </button>
                </div>
        ...
        <div class="px-6 py-4 border-t border-slate-100 shrink-0 flex justify-between items-center">
            <p class="text-xs text-slate-500">
                <span id="selectedCount2" class="font-semibold text-slate-900">0</span> products selected
            </p>
            <div class="flex gap-3">
                ...
                <button onclick="saveProducts()"
                    class="px-5 py-2 bg-red-600 text-white rounded-xl text-xs font-semibold hover:bg-red-700 transition cursor-pointer flex items-center gap-2">
...
<script>
    ...
    function renderSelectedProducts() {
        ...
        <div class="flex items-center gap-2.5 p-2.5 rounded-xl border border-slate-100 bg-slate-50/50 group">
            <span class="text-[10px] font-semibold text-slate-400 w-4">${i + 1}</span>
