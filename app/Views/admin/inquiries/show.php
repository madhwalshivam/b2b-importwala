...
<div class="space-y-5 font-sans pb-8">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
        <a href="<?= url('admin/inquiries') ?>"
            class="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-600 hover:text-[#f05a29] transition">
    ...
    <!-- Header Summary Card -->
    <div
        class="bg-white p-5 rounded-2xl border border-gray-200/80 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
            <div class="flex flex-wrap items-center gap-3">
                <h1 class="text-xl font-semibold text-slate-900">
                    Inquiry: <span
                        class="text-[#f05a29] font-mono font-semibold"><?= htmlspecialchars($inquiry['inquiry_number']) ?></span>
                </h1>
                <span
                    class="inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full border text-xs font-semibold <?= $badgeClass ?>">
        ...
        <!-- Status Change Form -->
        <form action="<?= url('admin/inquiries/update-status/' . $inquiry['id']) ?>" method="POST"
            class="flex items-center gap-2 bg-slate-50 p-2 border border-slate-200 rounded-xl">
            <label class="text-[11px] font-semibold text-slate-600 px-2 uppercase tracking-wider">Update Status:</label>
            <select name="status" onchange="this.form.submit()"
                class="px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-semibold text-slate-900 focus:outline-none focus:border-[#f05a29] cursor-pointer">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div class="lg:col-span-4 space-y-5">
            ...
            <div class="bg-white p-5 rounded-2xl border border-gray-200/80 shadow-xs space-y-4">
                <h3
                    class="text-xs font-semibold text-slate-900 border-b border-slate-100 pb-3 uppercase tracking-wider flex items-center gap-2">
                    <i data-lucide="user" class="w-4 h-4 text-[#f05a29]"></i> Customer Information
                </h3>

                <div class="space-y-3 text-xs">
                    <div>
                        <span class="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block">Full
                            Name</span>
                        <span
                            class="text-sm font-semibold text-slate-900 block mt-0.5"><?= htmlspecialchars($inquiry['customer_name']) ?></span>
                    </div>

                    <div>
                        <span class="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block">Phone /
                            WhatsApp</span>
                        <span
                            class="text-xs font-semibold text-slate-900 block mt-0.5"><?= htmlspecialchars($inquiry['phone']) ?></span>
                    </div>

                    <?php if (!empty($inquiry['email'])): ?>
                        <div>
                            <span class="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block">Email
                                Address</span>
                            <span
                                class="text-xs font-medium text-slate-700 block mt-0.5"><?= htmlspecialchars($inquiry['email']) ?></span>
                        </div>
                    <?php endif; ?>

                    <?php if (!empty($inquiry['company_name'])): ?>
                        <div>
                            <span class="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block">Company /
                                Firm</span>
                            <span
                                class="text-xs font-semibold text-slate-900 block mt-0.5"><?= htmlspecialchars($inquiry['company_name']) ?></span>
                        </div>
                    <?php endif; ?>

                    <?php if (!empty($inquiry['business_type'])): ?>
                        <div>
                            <span class="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block">Business
                                Type</span>
                            <span
                                class="inline-block mt-1 px-2.5 py-0.5 bg-slate-100 text-slate-800 text-[11px] font-semibold rounded-md">
                    ...
                    <?php if (!empty($inquiry['city']) || !empty($inquiry['state'])): ?>
                        <div>
                            <span
                                class="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block">Location</span>
                            <span
                                class="text-xs font-medium text-slate-700 block mt-0.5"><?= htmlspecialchars(trim($inquiry['city'] . ', ' . $inquiry['state'], ', ')) ?></span>
                        </div>
                    <?php endif; ?>
                    ...
                    <?php if (!empty($inquiry['gst_number'])): ?>
                        <div>
                            <span
                                class="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block">GSTIN</span>
                            <span
                                class="text-xs font-mono font-semibold text-slate-800 bg-slate-100 px-2 py-1 rounded inline-block mt-1">
                    ...
                    <?php if (!empty($inquiry['customer_message'])): ?>
                        <div class="pt-2">
                            <span
                                class="text-[10px] font-semibold text-slate-400 uppercase tracking-wider block mb-1">Customer
                                Message</span>
                            <div
                                class="p-3 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-700 leading-relaxed font-normal">
            ...
            <div class="bg-white p-5 rounded-2xl border border-gray-200/80 shadow-xs space-y-3">
                ...
                <form action="<?= url('admin/inquiries/update-notes/' . $inquiry['id']) ?>" method="POST"
                    ...
                    <textarea name="admin_notes" rows="4"
                        placeholder="Add private staff notes, quotation details, or discussion logs..."
                    ...
                    <button type="submit"
                        class="w-full py-2 bg-slate-900 hover:bg-slate-800 text-white font-semibold text-xs rounded-xl shadow-xs transition flex items-center justify-center gap-2 cursor-pointer">
            ...
            <div class="bg-white rounded-2xl border border-gray-200/80 shadow-xs overflow-hidden">
                <div class="px-5 py-3.5 border-b border-gray-100 bg-white flex items-center justify-between">
                    ...
                    <div class="text-xs font-semibold text-[#f05a29]">
                        <?= $inquiry['total_products'] ?> Products &bull;
                        <?= number_format($inquiry['total_quantity']) ?> Total Units
                    </div>
                </div>
                            ...
                            <?php foreach ($inquiry['items'] as $item): ?>
                                ...
                                <tr class="hover:bg-slate-50/50 transition">
                                    <td class="py-3.5 px-5">
                                        <div
                                            class="w-12 h-12 rounded-xl overflow-hidden border border-gray-200 bg-slate-50 shrink-0">
                                    ...
                                    <td class="py-3.5 px-5">
                                        <div class="font-semibold text-slate-900 text-xs">
                                            <?php if (!empty($item['current_product_slug'])): ?>
                                                <a href="<?= url('product/' . $item['current_product_slug']) ?>" target="_blank"
                                                    class="hover:text-[#f05a29] transition inline-flex items-center gap-1">
