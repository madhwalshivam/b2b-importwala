...
<div class="space-y-5 font-sans pb-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-5 rounded-2xl border border-gray-200/80 shadow-xs">
        <div>
            ...
            <p class="text-xs text-gray-500 font-medium mt-0.5">Manage multi-product wholesale requirements, quotations, and customer follow-ups.</p>
        </div>
        <?php if (!empty($newCount) && $newCount > 0): ?>
            <div class="shrink-0">
                <span class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-rose-500 text-white text-xs font-semibold rounded-xl shadow-xs animate-pulse">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="bg-white p-4 rounded-2xl border border-gray-200/80 shadow-xs flex items-center justify-between">
            <div>
                <span class="text-[11px] font-semibold text-slate-500 uppercase tracking-wider block">Total Inquiries</span>
        ...
        <div class="bg-white p-4 rounded-2xl border border-gray-200/80 shadow-xs flex items-center justify-between">
            <div>
                <span class="text-[11px] font-semibold text-slate-500 uppercase tracking-wider block">Action Required</span>
        ...
        <div class="bg-white p-4 rounded-2xl border border-gray-200/80 shadow-xs flex items-center justify-between">
            <div>
                <span class="text-[11px] font-semibold text-slate-500 uppercase tracking-wider block">Pipeline Status</span>
        ...
        <div class="bg-white p-4 rounded-2xl border border-gray-200/80 shadow-xs flex items-center justify-between">
            <div>
                <span class="text-[11px] font-semibold text-slate-500 uppercase tracking-wider block">Sourcing Channel</span>
                <span class="text-xs font-bold text-emerald-600 mt-0.5 block">Verified Wholesale</span>
            </div>
    ...
    <div class="bg-white p-4 rounded-2xl border border-gray-200/80 shadow-xs">
        <form action="<?= url('admin/inquiries') ?>" method="GET" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-3 items-end">
            
            <div class="lg:col-span-4">
                <label class="block text-[11px] font-semibold text-slate-600 mb-1 uppercase tracking-wider">Search Inquiries</label>
                <div class="relative">
                    <input type="text" name="search" value="<?= htmlspecialchars($search ?? '') ?>" placeholder="Search ID, Customer, Phone, Email..."
            ...
            <div class="lg:col-span-2">
                <label class="block text-[11px] font-semibold text-slate-600 mb-1 uppercase tracking-wider">Business Type</label>
                <select name="business_type" class="w-full h-9 px-3 bg-slate-50 border border-gray-200 rounded-xl text-xs font-semibold text-slate-800 focus:outline-none focus:border-[#f05a29] focus:bg-white transition cursor-pointer">
            ...
            <div class="lg:col-span-2">
                <label class="block text-[11px] font-semibold text-slate-600 mb-1 uppercase tracking-wider">Date From</label>
            ...
            <div class="lg:col-span-2 flex gap-2">
                ...
                <a href="<?= url('admin/inquiries') ?>" class="h-9 px-3 bg-slate-100 hover:bg-slate-200 text-slate-600 text-xs font-bold rounded-xl transition flex items-center justify-center cursor-pointer" title="Reset Filters">
                        ...
                        <?php foreach ($inquiries as $inq): ?>
                            ...
                            <tr class="hover:bg-slate-50/60 transition group">
                                ...
                                <td class="py-3.5 px-5">
                                    <div class="font-semibold text-slate-900 text-xs"><?= htmlspecialchars($inq['customer_name']) ?></div>
                                    <?php if (!empty($inq['company_name']) || !empty($inq['business_type'])): ?>
                                        <div class="text-[11px] font-normal text-slate-500 mt-0.5 truncate max-w-[240px]">
                                            ...
                                            <?php if (!empty($inq['business_type'])): ?>
                                                &bull; <span class="font-medium text-slate-700"><?= htmlspecialchars($inq['business_type']) ?></span>
                                            <?php endif; ?>
                                        </div>
                                    <?php endif; ?>
                                </td>
                                    ...
                                    <div class="flex items-center gap-1.5">
                                        ...
                                        <?php if (!empty($cleanPhone)): ?>
                                            <a href="https://wa.me/<?= $cleanPhone ?>?text=Hi%20<?= urlencode($inq['customer_name']) ?>%2C%20regarding%20your%20ImportWale%20Inquiry%20<?= urlencode($inq['inquiry_number']) ?>"
                                                ...
                                                class="text-emerald-600 hover:text-emerald-700 transition" title="WhatsApp Customer">
                                                <i data-lucide="message-circle" class="w-4 h-4 inline"></i>
                                            </a>
                                        <?php endif; ?>
                                    </div>
                                    <?php if (!empty($inq['email'])): ?>
                                        <div class="text-[11px] text-slate-400 font-normal truncate max-w-[180px]"><?= htmlspecialchars($inq['email']) ?></div>
                                    <?php endif; ?>
                                </td>
                                ...
                                <td class="py-3.5 px-5 text-center whitespace-nowrap">
                                    <span class="font-medium text-slate-800 text-xs">
                                        <?= $inq['total_products'] ?> <?= $inq['total_products'] == 1 ? 'Product' : 'Products' ?>
                                    </span>
                                </td>
                                ...
                                <!-- Status Badge -->
                                <td class="py-3.5 px-5 whitespace-nowrap">
                                    <span class="inline-flex items-center px-2.5 py-0.5 text-[11px] font-semibold rounded-full border <?= $stClass ?>">
