    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-xs">
        <div>
            <div class="flex items-center space-x-2">
                <a href="<?= url('admin/blogs') ?>" class="text-xs text-slate-500 hover:text-red-600 font-medium transition flex items-center space-x-1">
                    <i data-lucide="arrow-left" class="w-3.5 h-3.5"></i>
                    <span>Back to Blogs</span>
                </a>
                <span class="text-slate-400 text-xs">•</span>
                <span class="px-2.5 py-0.5 text-[11px] font-semibold uppercase bg-red-50 text-red-600 rounded-md border border-red-100">
        ...
        <div class="flex items-center space-x-3">
            <a href="<?= url('admin/blogs') ?>" class="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold rounded-xl transition">
                Cancel
            </a>
            <button type="submit" form="blog-form" class="px-5 py-2 bg-red-600 hover:bg-red-700 text-white text-xs font-semibold rounded-xl shadow-xs transition flex items-center space-x-2">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-4">
                    <div>
                        <label for="blog_title" class="block text-xs font-semibold text-slate-800 uppercase tracking-wider mb-1.5">
                            Blog Display Title <span class="text-red-500">*</span>
                        </label>
                        <input type="text" name="title" id="blog_title" value="<?= htmlspecialchars($post['title'] ?? '') ?>" required placeholder="e.g. ERW Pipe vs Seamless Pipe" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium focus:bg-white focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent transition">
                    </div>

                    <!-- Clean URL Slug Field with Live Preview -->
                    <div>
                        <div class="flex items-center justify-between mb-1">
                            <label for="blog_slug" class="block text-xs font-semibold text-slate-800 uppercase tracking-wider">
                        ...
                        <div class="relative flex items-center">
                            <span class="px-3 py-2 bg-slate-100 border border-r-0 border-slate-200 rounded-l-xl text-xs text-slate-500 font-mono select-none">
                                <?= url('blog/') ?>
                            </span>
                            <input type="text" name="slug" id="blog_slug" value="<?= htmlspecialchars($post['slug'] ?? '') ?>" placeholder="erw-pipe-vs-seamless-pipe" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-r-xl text-xs font-mono text-slate-800 focus:bg-white focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent transition">
                        </div>
                        <p class="text-[11px] text-slate-400 mt-1.5 flex items-center space-x-1">
                            ...
                            <span>URL format: Clean, lowercase, hyphens only. Duplicate slugs will auto-append numbers.</span>
                        </p>
                    </div>
                </div>
                ...
                <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-3">
                    <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                        <div>
                            ...
                            <p class="text-xs text-slate-500">Format headings (H1, H2, H3), lists, tables, internal/external links, and images with alt text.</p>
                        </div>
                    </div>
                    <div>
                ...
                <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-2">
                    <div class="flex items-center justify-between">
                        <label for="excerpt" class="block text-xs font-semibold text-slate-800 uppercase tracking-wider">
                            Short Excerpt / Summary
                        </label>
                        <span class="text-[11px] text-slate-400">Optional (Auto-generated from body if left blank)</span>
                    </div>
                    <textarea name="excerpt" id="excerpt" rows="3" placeholder="Brief summary of the article shown on blog listing cards..." class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:bg-white focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent transition"><?= htmlspecialchars($post['excerpt'] ?? '') ?></textarea>
                </div>
                ...
                <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-5">
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1.5">
                            <label for="meta_title" class="block text-xs font-semibold text-slate-800 uppercase tracking-wider">
                                Meta Title
                            </label>
                        ...
                        <input type="text" name="meta_title" id="meta_title" value="<?= htmlspecialchars($post['meta_title'] ?? '') ?>" placeholder="SEO Title for search engines (Recommended 50-60 characters)" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:bg-white focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent transition">
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1.5">
                            <label for="meta_description" class="block text-xs font-semibold text-slate-800 uppercase tracking-wider">
                                Meta Description
                            </label>
                        ...
                        <textarea name="meta_description" id="meta_description" rows="3" placeholder="Snippet summary shown in Google search results (Recommended 150-160 characters)" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:bg-white focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent transition"><?= htmlspecialchars($post['meta_description'] ?? '') ?></textarea>
                    </div>

                    <!-- Focus Keyword Field -->
                    <div>
                        <label for="focus_keyword" class="block text-xs font-semibold text-slate-800 uppercase tracking-wider mb-1.5">
                            Focus Keyword <span class="text-slate-400 font-normal lowercase">(optional reference)</span>
                        </label>
                        <input type="text" name="focus_keyword" id="focus_keyword" value="<?= htmlspecialchars($post['focus_keyword'] ?? '') ?>" placeholder="e.g. erw pipe vs seamless pipe" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:bg-white focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent transition">
                    </div>

                    <!-- Live Google Search Preview Box -->
                    <div class="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-1">
                        <p class="text-[11px] font-semibold uppercase text-slate-400 tracking-wider">Google Search Result Preview</p>
                        <div class="text-blue-700 text-sm font-semibold hover:underline cursor-pointer truncate" id="seo_preview_title">
                            <?= htmlspecialchars(($post['meta_title'] ?? '') ?: (($post['title'] ?? '') ?: 'Blog Title Preview')) ?> | Mudsor
                        </div>
                        <div class="text-emerald-700 text-[11px] font-mono truncate" id="seo_preview_url">
                            <?= url('blog/') ?><span id="seo_preview_slug_text"><?= htmlspecialchars(($post['slug'] ?? '') ?: 'your-blog-slug') ?></span>
                        </div>
            ...
            <div class="space-y-6">
                ...
                <!-- Publishing Box -->
                <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-5">
                    <h2 class="text-sm font-semibold text-slate-900 border-b border-slate-100 pb-3 flex items-center space-x-2">
                    ...
                    <!-- Status Dropdown -->
                    <div>
                        <label for="status" class="block text-xs font-semibold text-slate-800 uppercase tracking-wider mb-1.5">
                            Status <span class="text-red-500">*</span>
                        </label>
                        <select name="status" id="status" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:bg-white focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent transition">
                            <option value="draft" <?= ($post['status'] ?? '') === 'draft' ? 'selected' : '' ?>>Draft (Saved Privately)</option>
                            <option value="published" <?= ($post['status'] ?? '') === 'published' ? 'selected' : '' ?>>Published (Live on Website)</option>
                        </select>
                    </div>

                    <!-- Author Name -->
                    <div>
                        <label for="author_name" class="block text-xs font-semibold text-slate-800 uppercase tracking-wider mb-1.5">
                            Author Name
                        </label>
                        <input type="text" name="author_name" id="author_name" value="<?= htmlspecialchars($post['author_name'] ?? $defaultAuthor) ?>" placeholder="Mudsor Team" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:bg-white focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent transition">
                    </div>

                    <?php if ($isEdit): ?>
                        <div class="pt-2 border-t border-slate-100 text-[11px] text-slate-400 space-y-1">
                            <div><strong class="text-slate-600">Created:</strong> <?= date('M d, Y h:i A', strtotime($post['created_at'])) ?></div>
                            <?php if (!empty($post['published_at'])): ?>
                                <div><strong class="text-slate-600">Published:</strong> <?= date('M d, Y h:i A', strtotime($post['published_at'])) ?></div>
                            <?php endif; ?>
                            <div><strong class="text-slate-600">Views:</strong> <?= number_format((int)($post['views'] ?? 0)) ?></div>
                        </div>
                    <?php endif; ?>

                    <div class="pt-3 border-t border-slate-100 flex flex-col gap-2">
                        <button type="submit" class="w-full py-2.5 bg-red-600 hover:bg-red-700 text-white text-xs font-semibold rounded-xl shadow-xs transition flex items-center justify-center space-x-2">
                ...
                <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-xs space-y-4">
                    <h2 class="text-sm font-semibold text-slate-900 dark:text-slate-100 border-b border-slate-100 dark:border-slate-700 pb-3 flex items-center space-x-2">
                    ...
                    <div class="p-3 bg-amber-50 dark:bg-amber-950/60 border border-amber-200 dark:border-amber-900/60 rounded-xl flex items-start space-x-2.5 text-xs text-amber-800 dark:text-amber-300">
                        <i data-lucide="ruler" class="w-4 h-4 text-amber-600 dark:text-amber-400 shrink-0 mt-0.5"></i>
                        <div>
                            <p class="font-bold text-[11px]">Recommended Size: 1200 × 675 px (16:9 Ratio)</p>
                            <p class="text-[10px] text-amber-700 dark:text-amber-400 font-normal mt-0.5 leading-tight">
                                Creating images at <strong>1200 × 675 px</strong> (or 800 × 450 px) ensures perfect fit with zero overflow or cropping across Homepage, Blog Grid &amp; Related Articles.
                            </p>
                        </div>
                    </div>
                    ...
                    <!-- Image File Input -->
                    <div>
                        <input type="file" name="featured_image" id="featured_image" accept="image/*" class="w-full text-xs text-slate-500 file:mr-3 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-slate-100 file:text-slate-700 hover:file:bg-slate-200 transition cursor-pointer">
                    </div>

                    <!-- Featured Image Alt Text Input Field -->
                    <div>
                        <label for="featured_image_alt" class="block text-xs font-semibold text-slate-800 uppercase tracking-wider mb-1.5">
                            Featured Image Alt Text <span class="text-slate-400 font-normal lowercase">(optional - defaults to title)</span>
                        </label>
                        <input type="text" name="featured_image_alt" id="featured_image_alt" value="<?= htmlspecialchars($post['featured_image_alt'] ?? '') ?>" placeholder="Descriptive alt text for accessibility &amp; SEO" class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:bg-white focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent transition">
                        <p class="text-[10px] text-slate-400 mt-1">Describes the image for search engines (auto-uses title if left empty).</p>
                    </div>
                </div>
