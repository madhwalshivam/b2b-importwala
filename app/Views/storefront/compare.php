...
<div class="bg-gray-50 py-8 min-h-screen border-b border-gray-200 font-sans overflow-x-hidden" x-data="{
    ...
    <div class="container mx-auto px-3 sm:px-4 space-y-8">
        ...
        <!-- Breadcrumbs & Header -->
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-gray-200 shadow-xs">
            <div>
                ...
                <h1 class="text-2xl font-bold text-gray-900 leading-tight">Side-by-Side Product Comparison</h1>
                <p class="text-xs text-gray-500 mt-1 font-medium">Compare prices, descriptions, and specifications of your selected EV accessories</p>
            </div>
        ...
        <div class="space-y-4">
            <h3 class="text-xs font-bold text-gray-700 uppercase tracking-wider flex items-center space-x-2">
        ...
        <?php if (empty($products) || count($products) < 2): ?>
            ...
            <div class="bg-white rounded-3xl p-8 sm:p-12 border border-gray-200 text-center space-y-5 max-w-2xl mx-auto my-6 shadow-sm">
                <div class="w-20 h-20 bg-red-50 text-red-600 rounded-full flex items-center justify-center mx-auto border-2 border-dashed border-red-200">
                    <i data-lucide="git-compare" class="w-10 h-10"></i>
                </div>
                <div class="space-y-2">
                    <h3 class="text-xl font-bold text-gray-900">Add at least 2 products to compare</h3>
                    <p class="text-xs text-gray-500 max-w-md mx-auto leading-relaxed">
                        Select scooter brands and products from the dropdown boxes above to dynamically view title, price, description, and specifications side-by-side.
                    </p>
                </div>
                <div class="pt-2">
                    <a href="<?= url('shop') ?>"
        ...
        <?php else: ?>
            ...
            <div class="space-y-4">
                ...
                <!-- Toolbar Controls -->
                <div class="bg-white p-4 rounded-2xl border border-gray-200 flex flex-wrap items-center justify-between gap-4 shadow-xs">
                    <div class="flex items-center space-x-4">
                        <label class="flex items-center space-x-2 text-xs font-semibold text-gray-700 cursor-pointer select-none">
                            <input type="checkbox" x-model="highlightDiff" class="w-4 h-4 rounded text-red-600 focus:ring-red-500 border-gray-300">
                ...
                <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
                    <div class="compare-table-scroll-wrapper overflow-x-auto" style="-webkit-overflow-scrolling: touch; max-width: 100%;">
                        <?php
                        ...
                        <table class="w-full text-left border-collapse" style="min-width: <?= $mobileMin ?>px;">
                            ...
                            <thead>
                                <tr>
                                    <th class="p-3 sm:p-4 w-28 sm:w-40 md:w-60 align-top bg-gray-900 text-white font-bold text-xs uppercase tracking-wider sticky top-0 left-0 z-30 shadow-md">
                                    ...
                                    <?php foreach ($products as $p): ?>
                                        ...
                                        ?>
                                        <th class="p-3 sm:p-5 w-40 sm:w-56 md:w-72 align-top border-l border-gray-800 bg-gray-900 text-white sticky top-0 z-20 shadow-md">
                                            <div class="space-y-3 relative">
                                                ...
                                                <!-- Product Main Image -->
                                                <div class="w-20 h-20 sm:w-28 sm:h-28 mx-auto bg-white rounded-xl p-1 sm:p-2 border border-gray-700 flex items-center justify-center shadow-xs overflow-hidden">
                                                ...
                                                <div class="text-center space-y-1">
                                                    <span class="text-[10px] font-bold text-red-400 uppercase tracking-wider block">
                                                        <?= htmlspecialchars($p['brand_name'] ?? 'Mudsor') ?>
                                                    </span>
                                                    <h3 class="text-xs sm:text-sm font-bold text-white line-clamp-2 leading-snug">
                                                        <a href="<?= url('product/' . $p['slug']) ?>" class="hover:text-red-400 transition">
                                                ...
                                                <div class="text-center space-y-1 pt-1 border-t border-gray-800">
                                                    ...
                                                    <?php if ($isLowestPrice): ?>
                                                        <span class="inline-block bg-emerald-600 text-white text-[10px] px-2.5 py-0.5 rounded-full font-bold uppercase tracking-wider shadow-xs">
                                                            🏆 Best Value
                                                        </span>
                                                    <?php endif; ?>
                                                </div>
                            ...
                            <tbody class="divide-y divide-gray-200 text-xs font-medium text-gray-800">
                                ...
                                <tr class="hover:bg-gray-50/50">
                                    <td class="p-2 sm:p-4 font-bold text-gray-900 bg-gray-100/60 uppercase tracking-wider sticky left-0 z-10 text-[10px] sm:text-xs w-28 sm:w-auto">
                                        Price &amp; Tax
                                    </td>
                                    <?php foreach ($products as $p): ?>
                                ...
                                <tr class="hover:bg-gray-50/50">
                                    ...
                                    <?php foreach ($products as $p): ?>
                                        <td class="p-2 sm:p-4 border-l border-gray-200 align-top text-gray-600 leading-relaxed text-[11px] sm:text-xs">
                                            <?php 
                                            ...
                                            <?php if (!empty($cleanDesc)): ?>
                                                ...
                                                <a href="<?= url('product/' . $p['slug']) ?>" class="inline-block text-[11px] font-semibold text-red-600 hover:underline mt-1.5">
                                ...
                                <?php if (!empty($specKeys)): ?>
                                    <?php foreach ($specKeys as $key): ?>
                                        ...
                                        ?>
                                        <tr :class="highlightDiff && !<?= $allSame ? 'true' : 'false' ?> ? 'bg-amber-50/70' : 'hover:bg-gray-50/50'">
                                            <td class="p-2 sm:p-4 font-bold text-gray-900 bg-gray-100/60 uppercase tracking-wider sticky left-0 z-10 text-[10px] sm:text-xs w-28 sm:w-auto">
            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center space-x-3 bg-white p-4 rounded-2xl border border-gray-200 shadow-xs">
                    <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-red-600 to-red-700 text-white flex items-center justify-center shadow-md shrink-0">
                        <i data-lucide="git-compare" class="w-5 h-5"></i>
                    </div>
                    <div>
                        <h2 class="text-sm font-bold text-gray-900 leading-tight">OEM vs Mudsor — Price & Value Comparison</h2>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <?php foreach ($products as $pIdx => $p):
                        ...
                        if ($oemWarranty <= 0) $oemWarranty = 6; // default OEM = 6 months
                    ...
                    <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm flex flex-col">
                        ...
                        <div class="bg-gray-900 px-4 py-3 flex items-center space-x-3">
                            <div class="w-10 h-10 bg-white rounded-lg overflow-hidden flex items-center justify-center p-0.5 shrink-0 border border-gray-800">
                                <img src="<?= str_starts_with($p['main_image'], 'http') ? $p['main_image'] : asset(ltrim($p['main_image'], '/')) ?>"
                                    alt="" class="max-w-full max-h-full object-contain">
                            </div>
                            <div class="min-w-0 flex-1">
                                <p class="text-[10px] text-red-400 font-bold uppercase tracking-wider">Product <?= $pIdx + 1 ?></p>
                                <h3 class="text-xs font-semibold text-white line-clamp-2 leading-snug"><?= htmlspecialchars($p['name']) ?></h3>
                            </div>
                        </div>

                        <!-- Column Headers -->
                        <div class="grid grid-cols-3 text-[10px] font-bold uppercase tracking-wider border-b border-gray-200">
                            <div class="col-span-1 p-2.5 bg-gray-100 text-gray-500">Feature</div>
                            <div class="col-span-1 p-2.5 bg-gray-50 text-gray-600 text-center border-l border-gray-200">
                                OEM / Company
                            </div>
                            <div class="col-span-1 p-2.5 bg-emerald-50 text-emerald-700 text-center border-l border-gray-200">
                                🏷️ Mudsor
                            </div>
                        </div>
                        ...
                        <div class="divide-y divide-gray-100 flex-1">
                            ...
                            <div class="grid grid-cols-3 text-xs">
                                <div class="col-span-1 px-3 py-3 bg-gray-50 font-bold text-gray-600 uppercase tracking-wider text-[10px] flex items-center">
                                ...
                                <div class="col-span-1 px-3 py-3 border-l border-gray-100 text-center">
                                    ...
                                    <?php if ($isOemEstimated): ?>
                                        <div class="text-[9px] text-orange-500 font-semibold mt-0.5">Est. market price</div>
                                    <?php else: ?>
                                        <div class="text-[9px] text-gray-400 mt-0.5">Company MRP</div>
                                    <?php endif; ?>
                                </div>
                                <div class="col-span-1 px-3 py-3 border-l border-gray-100 text-center bg-emerald-50/60">
                            ...
                            <div class="grid grid-cols-3 text-xs">
                                <div class="col-span-1 px-3 py-3 bg-gray-50 font-bold text-gray-600 uppercase tracking-wider text-[10px] flex items-center">
                                    <i data-lucide="shield-check" class="w-3 h-3 mr-1 text-gray-400 shrink-0"></i>Warranty
                                </div>
                                <div class="col-span-1 px-3 py-3 border-l border-gray-100 text-center">
                                    <div class="font-bold text-gray-700"><?= $oemWarranty ?> Months</div>
                                ...
                                <div class="col-span-1 px-3 py-3 border-l border-gray-100 text-center bg-emerald-50/60">
                                    ...
                                    <?php if ($warrantyMultiplier !== null && $warrantyMultiplier > 1): ?>
                                        <div class="text-[9px] text-emerald-600 font-bold mt-0.5">✅ <?= $warrantyMultiplier ?>x More</div>
                                    <?php endif; ?>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-3 text-xs">
                                ...
                                <div class="col-span-1 px-3 py-3 border-l border-gray-100 text-center">
                                    <div class="font-semibold text-gray-600 leading-snug text-[11px]"><?= htmlspecialchars($oemMaterial) ?></div>
                                </div>
                                <div class="col-span-1 px-3 py-3 border-l border-gray-100 text-center bg-emerald-50/60">
                                    <div class="font-semibold text-emerald-800 leading-snug text-[11px]"><?= htmlspecialchars($mudsorMaterial) ?></div>
                                    <div class="text-[9px] text-emerald-600 font-bold mt-0.5">✅ Upgraded</div>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-3 text-xs">
                                ...
                                <div class="col-span-1 px-3 py-3 border-l border-gray-100 text-center">
                                    <div class="font-semibold text-gray-600 leading-snug text-[11px]"><?= htmlspecialchars($oemFinish) ?></div>
                                </div>
                                <div class="col-span-1 px-3 py-3 border-l border-gray-100 text-center bg-emerald-50/60">
                                    <div class="font-semibold text-emerald-800 leading-snug text-[11px]"><?= htmlspecialchars($mudsorFinish) ?></div>
                                    <div class="text-[9px] text-emerald-600 font-bold mt-0.5">✅ Better</div>
                                </div>
                            </div>
                        ...
                        <div class="bg-gradient-to-r from-emerald-600 to-emerald-700 px-4 py-2.5 flex items-center justify-between">
                            <div class="flex items-center space-x-1.5">
                                <i data-lucide="trophy" class="w-3.5 h-3.5 text-yellow-300"></i>
                                <span class="text-white text-[11px] font-bold">
                                    <?php
                                        $wins = [];
                                        if ($priceSaving > 0) $wins[] = format_price($priceSaving) . ' sasta';
                                        if ($warrantyMultiplier > 1) $wins[] = $warrantyMultiplier . 'x warranty';
                ...
                <p class="text-[10px] text-gray-400 text-center font-medium">
                    * OEM = Original Equipment Manufacturer — vehicle brand (Ola, TVS, Honda, etc.) ka original spare part price.
